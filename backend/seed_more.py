...
BUS_COMPANIES = ["MR Express","BlueLine Express","SwiftCoach","EagleTransit","CoastalRider","PacificCoach","MidwestExpress","SouthernLines","GreenLine Bus","StarBus"]
...
for origin in cities:
    for destination in cities:
        if origin == destination:
            continue
        # Add 7 more buses per route (total will be 10)
        for j in range(3, 10):
            rng = random.Random(int(hashlib.md5(f"{origin}{destination}{j}extra".encode()).hexdigest(), 16) % 10**8)
        ...
        if total % 10000 == 0:
            db.commit()
            print(f"Added {total} more buses...")

db.commit()
print(f"Done! Added {total} more buses — now 10 per route!")
db.close()
